...
- android-agent / ios-agent: before implementation, grep their own repo with these keywords → classify each match as **reuse** (import existing as-is) / **extend** (modify or add to existing) / **new** (create from scratch) / **remove** (delete deprecated) → record one line in the PR body (`Inventory: reuse X / extend Y / new Z / remove W`). They also report two facts only their repo holds: `Idiom:` — what the target screen is actually built with — and `Assets:` — the form each named asset is in, and the constraint that follows (a badge-form icon that cannot take a tint; an intrinsic size that does not match the box the spec assumes). Both are per-repo facts you cannot supply and must not guess.
- pm-agent final review: a document-level pass over the two PR bodies / issue bodies / `_tasks` (no platform-source reading) — compare the one-line `Inventory:` and `Assets:` summaries and the spec-term behavior descriptions, flag unintended divergence. (`Idiom:` is recorded, not reconciled — the platforms build screens differently by nature.) See §Cross-platform consistency review.
...
You also receive a **scope reconciliation**: `/feat` runs the *lagging* platform agent's §Scope cross-check mode against the brief, so you can size the lagging issue to *that* repo's reality — each brief item classified already-present / to-build / adapt / n/a, plus **lagging-only additions** the reference didn't need. Both inputs are inline and transient. The brief is the *candidate* scope (what the reference changed); the reconciliation is the *actual* lagging scope (what this repo needs).
...
5. **Size the lagging scope from the reconciliation, not the reference's raw blast radius.** Use the reconciliation from the lagging agent's §Scope cross-check mode to set the issue scope: **already-present** items → note as reuse, *not* a task in the issue; **to-build / adapt / lagging-only additions** → in the lagging issue scope + `## Completion checklist`; `decision`-flagged items → `## Open decisions`. The brief sized the candidate scope (what the reference changed); the reconciliation is the actual lagging scope (what this repo needs) — author from the latter. (If no reconciliation was supplied — e.g. pm-agent was invoked directly, not via `/feat` — say so in the issue: the scope is reference-estimated, and the lagging agent must cross-check it against its repo before implementing.)
6. **Parity gaps → `## Open decisions`.** States the reference itself didn't handle (the brief flags them) are decisions for the lagging platform / PM — not behavior to silently copy, and not gaps to invent a fix for.
7. **One issue — the lagging platform only.** The reference is already done; do not re-issue it. The dry-run / live-creation (Steps 5–7) produces a **single** issue, for the lagging platform. Its body is self-contained as always, plus one line: "Parity target — match {reference}'s shipped behavior described above." In the `## Completion checklist`, the reference platform's line carries **no issue number** — write it `{reference} implementation (reference — already shipped)`; only the lagging platform's line gets a `#{N}` (mirrors the header Issue-line handling above).
...
- **Asset-form divergence** — compare the two `Assets:` lines. If one platform reports an asset it cannot tint (or whose intrinsic size fights the spec's box) and the other reports no such constraint, the screens will not match no matter how faithfully each followed the spec. Flag it. Do **not** flag a difference between the two `Idiom:` lines — the platforms legitimately build screens differently, and that line exists to record what each did, not to be reconciled.
