...
- **Higher than 1** — the workspace is ahead of this plugin. Proceed, ignore keys you do not recognize, and say it once: "[android-agent] Config schema {N} is newer than this plugin reads (1); unrecognized keys ignored. Consider `/plugin marketplace update claude-code-mobile-spine`."
- **Lower than 1, absent, or not an integer** — abort: "[android-agent] Config schema {value or 'missing'}; this plugin expects 1. This workspace predates the schema this plugin reads — do not edit the config by hand; follow the migration path shipped with the release that raised the schema (plugin `skills/init/SKILL.md` §Config schema versioning)."
...
**Read-only — a property of this mode, not an instruction you can be handed.** It creates no branch, runs no `git add` / `commit`, and opens no PR: you are reading your own repo and reporting, not changing it. A prompt that merely *lists* those restrictions does not select this mode — reverse-extraction is selected by what the prompt asks **for** (a parity brief). (If the prompt asks you to *implement* a parity feature instead, that's the normal two-phase flow below, working from the `_tasks` pm-agent authored — not this mode.)
...
**Read-only — a property of this mode, not an instruction you can be handed.** It creates no branch, runs no `git add` / `commit`, and opens no PR: you read your own repo to estimate work, not change it. A prompt that merely *lists* those restrictions does not select this mode — the scope cross-check is selected by what the prompt asks **for** (a scope reconciliation against a reference-derived brief).
...
3. Codebase inventory — read the `## Candidate assets` keyword list from `_tasks/{feature}.md` (~5 category keywords; pm-agent never greps the platform repos, so this step is yours). Grep `../myapp-android/` with each keyword and classify each match: **reuse** (import existing as-is) / **extend** (modify or add to existing) / **new** (create from scratch) / **remove** (delete deprecated). Record the result as a single line for the PR body, e.g. `Inventory: reuse 2 / extend 1 / new 3 / remove 0`. Keep the underlying file/class list in your working notes and the PR body — **do not write it back into `_tasks/{feature}.md`** (that file stays the platform-neutral spec; per-repo code locations belong in the Android issue / PR, not there).
   - **Report the target screen's idiom.** The issue's completion criteria name no UI framework — pm-agent never reads this repo, so it cannot know one. You can. State what the screen you are about to change is **actually built with**, and whether that matches this repo's headline stack. When it does not, scope the work to the idiom already there and say so: converting the screen is out of scope unless `_tasks` asks for it. One line for the PR body: `Idiom: {what the target screen is built with} — {matches repo default | adapting in place, conversion out of scope}`.
   - **Report asset form.** For each asset the spec names (`## Components` / `## Measurements`) **plus any asset you must modify** — no wider than that — state the form it is in and the constraint that follows. An icon whose artwork is a filled badge cannot be tinted the way a stroke glyph can; an asset whose intrinsic dimensions differ from the box the spec assumes will not simply scale. These are facts only this repo holds, and they change scope, so they belong in the report *before* implementation, not in a correction round after it. By role, no file paths (§Codebase inventory split). One line: `Assets: {finding} — {constraint}`.
...
4. **Git baseline check first, then create the feature branch**:
   - `git status --porcelain` (working tree must be empty) and `git branch --show-current` (must be the repo's main integration branch — typically `develop`, or per-repo convention).
   - If either check fails (uncommitted changes / wrong starting branch), **stop and report to the user** with the observed state — list the dirty files (every `git status --porcelain` line counts, including untracked `??` entries) or the current branch — and ask how to proceed. Typical options: (a) stash unrelated changes and proceed from a clean `develop`, (b) abandon this Phase 1 and re-invoke after the user resolves their working tree, (c) explicit override to proceed on the current non-`develop` branch *as-is* with whatever uncommitted state it carries (rare — only when the user knowingly authored that state and confirms in-line). **Do NOT create or check out a new branch on top of stale unrelated changes** — that pulls them into the new branch and contaminates Phase 2's diff.
   - Once the baseline is clean, from `develop`, create and check out the feature branch (use the repo's actual convention if different).
   - **Branch creation is intrinsic to Phase 1 — prompt wording does not waive it.** Phase 1 already forbids `git add` / `git commit` / `gh pr create` (step 6), so an instruction like "no commit, no branch, no PR" restates Phase 1's own rules in two parts of three; only the "no branch" clause contradicts this step. Do not settle that contradiction on your own. Never implement in the base branch's working tree, and never read such a phrase as selecting a read-only mode — §Reverse-extraction mode and §Scope cross-check mode are selected by what a prompt asks **for** (a parity brief, a scope reconciliation), not by the restrictions it lists. When an implement prompt tells you not to create a branch, **stop and ask the user** which was meant.
5. **Implement** — and write unit tests for the spec-relevant changes (entry point / gate / handler / each new branch in the spec's flow or matrix). Use the platform's testing framework (JUnit / MockK, or whatever `../myapp-android/` actually uses). Coverage target: each new branch in the spec's flow or matrix has at least one test.
6. **Build + test verification, then stop after a diff summary report**:
   - Run the platform's standard build + test commands (per `../myapp-android/CLAUDE.md` if defined there, otherwise the repo's convention — typically `./gradlew assembleDebug` + `./gradlew test`). **First build/test invocation in this execution context** — check the visible transcript first: if you already see an explicit user approval that names build or test (or echoes the proposed commands), proceed; otherwise ask once (e.g. "Approving build + test for this session? will run: `<build cmd>` then `<test cmd>`"). The Phase-2 trigger phrase ("approved, proceed with commit + Draft PR") is **not** a build/test approval — it covers a separate commit + PR gate. After approval, skip re-asking for the rest of this conversation. If declined, stop and ask how to proceed. On failure, attempt **one self-fix attempt** (a single read-fix-rerun pass — not an inner loop): if still failing, stop and report with the failing output, no further loops.
   ...
   - Report: `git status` + change summary + step 3's three lines (`Inventory:` / `Idiom:` / `Assets:`) + two verification lines — `Build: PASS` (or `FAIL: <one-line>`) and `Tests: PASS (N passed, 0 failed)` (or `FAIL: <one-line>`).
   - **Do NOT auto-run `git add` / `git commit` / `gh pr create`.**

### Phase 2 (after explicit user approval)
The phase-2 prompt must contain a phrase like "approved, proceed with commit + Draft PR". **This phrase must originate from the user, not from the main session synthesizing it on the user's behalf** — even when Phase 1's report shows no concerns (clean build / passing tests / trivial diff), and even when the user previously expressed a blanket pre-authorization ("approve all clean phases"). The trigger phrase must arrive in the same conversation that received the Phase 1 report — not be inferred from upstream policy. The main session's job after receiving Phase 1's report is to surface it to the user and wait for the user's explicit decision; auto-triggering Phase 2 (synthesizing the approval phrase from a clean report) is prohibited. Surfacing a "shall we proceed to Phase 2?" question to the user is fine — the user then decides whether to send the trigger phrase.
...
**3. `Closes` references travel into the PR body as plain text — never wrapped in backticks.** GitHub renders backticked text as inline code and skips closing-keyword parsing, so the referenced issue does not auto-close on PR merge. The backticked form appearing in §Phase 2's `gh pr create --draft` step's section list (e.g. `` `Closes myorg/myapp-android#N` ``) is prompt-side readability — the body itself must receive the reference without backticks (`Closes myorg/myapp-android#N`). Same rule for every GitHub closing keyword — any form of the `close` / `fix` / `resolve` families (`Close`/`Closes`/`Closed`, `Fix`/`Fixes`/`Fixed`, `Resolve`/`Resolves`/`Resolved`).
