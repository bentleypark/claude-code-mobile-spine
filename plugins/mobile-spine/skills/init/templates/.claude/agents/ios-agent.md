...
6. Implement.
7. **Stop after a diff summary report** (`git status` + change summary). **Do NOT auto-run `git add` / `git commit` / `gh pr create`.**
...
10. `gh pr create --draft --base develop` — self-contained PR body (change summary / self-inventory / per-repo Figma procedure outcome / Pod changes / known limitations / test scenarios / `Closes myorg/myapp-ios#N`). **Do NOT add `🤖 Generated with Claude Code`-type footers.**
